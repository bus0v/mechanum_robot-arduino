...
// time
  long t1 = micros();
  target[0] = 50*(sin(t1/1e6)>0);
  target[1] = 50*(sin(t1/1e6)>0);
  target[2] = 50*(sin(t1/1e6)>0);
  target[3] = 50*(sin(t1/1e6)>0);
...
for (int k=0; k<4; k++){
  ...
  //get speed in cm/s
  vcm[k] = velocity[k]/330*25.1;
  // Low-pass filter (25 Hz cutoff)
  //vFilt[k] = 0.854*vFilt[k] + 0.0728*vcm[k] + 0.0728*vPrev[k];
  vFilt[k] = 0.969*vFilt[k] + 0.0155*vcm[k] + 0.0155*vPrev[k];
  vPrev[k] = vFilt[k];
